...
static float score_frame_damage_aware(
    ...
    // NOTE: Removed codon scoring (redundant with dicodon), redistributed to dicodon
    return 0.28f * stop + 0.10f * aa_comp +
           0.28f * dicodon + 0.13f * dipeptide +
...
std::pair<FrameScore, FrameScore> FrameSelector::select_best_per_strand_damage_aware(
    ...
    // Get length-adaptive weights
    LengthAdaptiveWeights w = get_length_adaptive_weights(seq.length());
    ...
    for (int frame = 0; frame < 3; ++frame) {
        ...
        if (!protein.empty()) {
            // Damage-adjusted stop score (considers damage probability at each stop)
            float damage_stop = calculate_damage_adjusted_stop_multiplier(seq, protein, frame, damage_model);

            // Phase 1.3: Poisson-based stop score for length normalization
            float stop_llr = calculate_poisson_stop_llr(protein);
            float poisson_stop = poisson_stop_to_score(stop_llr, seq.length());

            // Blend damage-adjusted and Poisson models
            // For long reads, Poisson provides better length normalization
            // For short reads with damage, damage-adjusted is more reliable
            float stop;
            if (seq.length() >= 130) {
                stop = 0.6f * poisson_stop + 0.4f * damage_stop;
            } else {
                stop = 0.4f * poisson_stop + 0.6f * damage_stop;
            }
            ...
            float wobble_bonus = (wobble_enrich - 0.5f) * 0.10f;

            // Phase 1.2: Use length-adaptive weights instead of hardcoded
            // Dynamic adjustment: when stop is perfect, redistribute to hexamer
            float effective_stop_weight = w.stop_weight;
            float effective_hexamer_weight = w.hexamer_weight + w.dicodon_weight;

            if (stop > 0.99f) {
                effective_stop_weight = 0.0f;
                effective_hexamer_weight += w.stop_weight;
            }

            float total = effective_stop_weight * stop +
                         w.aa_weight * aa_comp +
                         effective_hexamer_weight * hexamer +
                         w.dipeptide_weight * dipeptide +
                         w.gc3_weight * gc3_bonus +
        ...
        if (!protein.empty()) {
            // Damage-adjusted stop score (considers damage probability at each stop)
            float damage_stop = calculate_damage_adjusted_stop_multiplier(rc_seq, protein, frame, damage_model);

            // Phase 1.3: Poisson-based stop score for length normalization
            float stop_llr = calculate_poisson_stop_llr(protein);
            float poisson_stop = poisson_stop_to_score(stop_llr, rc_seq.length());

            // Blend damage-adjusted and Poisson models
            float stop;
            if (rc_seq.length() >= 130) {
                stop = 0.6f * poisson_stop + 0.4f * damage_stop;
            } else {
                stop = 0.4f * poisson_stop + 0.6f * damage_stop;
            }
            ...
            // Phase 1.2: Use length-adaptive weights
            float effective_stop_weight = w.stop_weight;
            float effective_hexamer_weight = w.hexamer_weight + w.dicodon_weight;
...
/**
 * Select best frame per strand using UnifiedDamageContext
 *
 * This version uses the unified damage architecture for consistent handling:
 ...
 * - Correct strand handling via for_reverse_complement()
 */
std::pair<FrameScore, FrameScore> FrameSelector::select_best_per_strand_damage_aware(
    const std::string& seq,
    const UnifiedDamageContext& ctx) {
    ...
    // Get damage parameters for hexamer weighting (fall back to defaults if no context)
    float delta_5 = ctx.enabled() ? ctx.d_max() : 0.0f;
    float delta_3 = ctx.enabled() ? ctx.d_max() : 0.0f;

    // Compute reverse complement ONCE
    const std::string& rc_seq = reverse_complement_fast(seq);

    // Create reverse complement context with swapped 5'/3' meaning
    UnifiedDamageContext ctx_rc = ctx.for_reverse_complement();
    ...
    // Score all 6 frames with unified damage context
    for (int frame = 0; frame < 3; ++frame) {
        // Forward frame
        protein = translate_sequence(seq, frame);
        if (!protein.empty()) {
            // Damage-adjusted stop score using UnifiedDamageContext
            float damage_stop = calculate_damage_adjusted_stop_multiplier(seq, protein, frame, ctx);

            // Poisson-based stop score for length normalization
            float stop_llr = calculate_poisson_stop_llr(protein);
            float poisson_stop = poisson_stop_to_score(stop_llr, seq.length());
            ...
            // Positional hexamer score with damage weighting
            float hexamer = positional::calculate_hexamer_score_damage_aware(seq, frame, delta_5, delta_3);
            ...
            float damage_consistency = score_damage_frame_consistency(seq, frame);
            float damage_bonus = 0.0f;
            if (seq.length() > 0 && ctx.enabled()) {
                bool has_damage_signal = (fast_upper(seq[0]) == 'T') ||
                                          (fast_upper(seq[seq.length()-1]) == 'A');
                if (has_damage_signal) {
                    damage_bonus = (damage_consistency - 0.5f) * 0.15f * ctx.damage_gate();
                }
            }

            // Wobble position T-enrichment
            float wobble_enrich = calculate_wobble_enrichment(seq, frame, 15);
            float wobble_bonus = (wobble_enrich - 0.5f) * 0.10f;
        ...
        // Reverse frame (uses ctx_rc with swapped 5'/3')
        protein = translate_sequence(rc_seq, frame);
        if (!protein.empty()) {
            // Damage-adjusted stop score using reverse complement context
            float damage_stop = calculate_damage_adjusted_stop_multiplier(rc_seq, protein, frame, ctx_rc);

            // Poisson-based stop score
            float stop_llr = calculate_poisson_stop_llr(protein);
            float poisson_stop = poisson_stop_to_score(stop_llr, rc_seq.length());
            ...
            float damage_consistency = score_damage_frame_consistency(rc_seq, frame);
            float damage_bonus = 0.0f;
            if (rc_seq.length() > 0 && ctx_rc.enabled()) {
                bool has_damage_signal = (fast_upper(rc_seq[0]) == 'T') ||
                                          (fast_upper(rc_seq[rc_seq.length()-1]) == 'A');
                if (has_damage_signal) {
                    damage_bonus = (damage_consistency - 0.5f) * 0.15f * ctx_rc.damage_gate();
                }
            }
