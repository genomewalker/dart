...
int cmd_damage_annotate(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        if ((strcmp(argv[i], "--hits") == 0 || strcmp(argv[i], "-i") == 0) && i + 1 < argc) {
            ...
            std::cerr << "Usage: agp damage-annotate -i <hits.tsv> [options]\n\n";
            ...
            std::cerr << "Required:\n";
            std::cerr << "  -i, --hits <file>       MMseqs2 results TSV with qaln/taln columns\n\n";
            std::cerr << "Options:\n";
            ...
            std::cerr << "  --d-max <float>         Override damage rate (default: estimated)\n";
            std::cerr << "  --lambda <float>        Decay constant (default: 0.3)\n";
            std::cerr << "  --max-dist <int>        Max nt from terminus, -1=off (default: -1)\n";
            std::cerr << "  --library-type <type>   Library type: ss, ds, or auto (default: auto)\n";
            std::cerr << "  --threshold <float>     Damage threshold (default: 0.7)\n";
