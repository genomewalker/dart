    ...
    with opener(gff_path, 'rt') as f:
        for line in f:
            ...
            scores[gene_id] = {
                ...
                'coding_prob': float(attrs.get('score', parts[5])) if parts[5] != '.' else 0.0,
...
def run_mmseqs_search(query_fasta: str, target_db: str, output_tsv: str,
    ...
    cmd = [
        'mmseqs', 'easy-search',
        query_fasta, target_db, output_tsv, tmp_dir,
...
def assign_pseudo_labels(scores: dict, hits: dict,
...
def build_calibration_table(labeled_data: dict,
                            score_bins: list,
                            length_bins: list,
                            damage_bins: list) -> np.ndarray:
    ...
    for gene_id, data in labeled_data.items():
        # Compute logit(p_correct) as score - but we use coding_prob for now
...
""")
        ...
        f.write(f"constexpr std::array<float, {len(score_bins)}> SCORE_EDGES_LOGIT = {{{{\n    ")
        f.write(", ".join(f"{x:.4f}f" for x in score_bins))
        f.write("\n}};\n\n")

        # Length bins
        f.write(f"// Length bins (bp)\n")
        f.write(f"constexpr std::array<float, {len(length_bins)}> LENGTH_EDGES_CALIB = {{{{\n    ")
        f.write(", ".join(f"{x:.1f}f" for x in length_bins))
        f.write("\n}};\n\n")

        # Damage bins
        f.write(f"// Damage bins (%)\n")
        f.write(f"constexpr std::array<float, {len(damage_bins)}> DAMAGE_EDGES_CALIB = {{{{\n    ")
        ...
        f.write(f"constexpr float CALIBRATION_PROTEIN[{n_score}][{n_length}][{n_damage}] = {{{{\n")
...
def main():
    ...
    parser.add_argument('--target-db', required=True, help='Target protein database (FASTA)')
    parser.add_argument('--output-header', default='score_calibration_protein.hpp',
                        help='Output C++ header file')
    parser.add_argument('--threads', type=int, default=16, help='Number of threads')
    parser.add_argument('--fdr', type=float, default=0.01, help='FDR threshold for pseudo-labels')
    parser.add_argument('--min-identity', type=float, default=0.8, help='Min identity for correct label')
    parser.add_argument('--min-coverage', type=float, default=0.5, help='Min coverage for correct label')
    parser.add_argument('--tmp-dir', default='/scratch/tmp/calibration', help='Temp directory')
    ...
    labeled = assign_pseudo_labels(scores, hits, args.fdr, args.min_identity, args.min_coverage)

    n_correct = sum(1 for d in labeled.values() if d['is_correct'])
    n_total = len(labeled)
    print(f"  Labeled: {n_total}, Correct: {n_correct} ({100*n_correct/max(n_total,1):.1f}%)")
    ...
    calibration, counts = build_calibration_table(labeled, score_bins, length_bins, damage_bins)
    ...
    generate_header(calibration, score_bins, length_bins, damage_bins, args.output_header)
