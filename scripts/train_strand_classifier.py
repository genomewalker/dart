...
def extract_strand_features(seq):
    ...
    for strand_seq in [seq, rc]:
        ...
        codons = [strand_seq[i:i+3] for i in range(best_frame, len(strand_seq)-2, 3)]
...
def export_mlp_to_cpp(model, scaler, output_path, n_features):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f" * Architecture: {input_size} -> {hidden_size} (ReLU) -> 1 (sigmoid)\n")
        f.write(" * \n")
        f.write(" * This classifier predicts whether the forward strand (+) contains\n")
        f.write(" * the correct reading frame. Output > 0.5 means forward is correct.\n")
        f.write(" */\n\n")
        ...
        f.write("    static const std::array<float, INPUT_SIZE> INPUT_MEAN = {{\n        ")
        ...
        f.write("    static const std::array<float, INPUT_SIZE> INPUT_STD = {{\n        ")
        ...
        # W1
        f.write("inline const std::array<std::array<float, HIDDEN_SIZE>, INPUT_SIZE>& get_W1() {\n")
        f.write("    static const std::array<std::array<float, HIDDEN_SIZE>, INPUT_SIZE> W1 = {{\n")
        ...
        f.write("inline int count_internal_stops(const std::string& seq, int frame) {\n")
        f.write("    int stops = 0;\n")
        ...
        f.write("        if (is_stop && i + 5 < seq.length()) stops++;  // internal only\n")
        f.write("    }\n")
        f.write("    return stops;\n")
        f.write("}\n\n")
        
        f.write("// Compute hexamer-like score\n")
        f.write("inline float compute_hexamer_score(const std::string& seq, int frame) {\n")
        ...
        f.write("            if (c != 'A' && c != 'C' && c != 'G' && c != 'T') { valid = false; break; }\n")
        ...
        f.write("        if ((pos1 == 'A' || pos1 == 'C' || pos1 == 'G' || pos1 == 'T') &&\n")
        f.write("            (pos3 == 'A' || pos3 == 'C' || pos3 == 'G' || pos3 == 'T')) {\n")
        f.write("            total++;\n")
        f.write("            if ((pos1 == 'A' || pos1 == 'G') && (pos3 == 'C' || pos3 == 'T')) count++;\n")
        f.write("        }\n")
        f.write("    }\n")
        ...
        f.write("inline void compute_gc_by_position(const std::string& seq, int frame, float gc_pos[3]) {\n")
        ...
        f.write("        if (score > best_score) { best_score = score; best = frame; }\n")
        f.write("    }\n")
        f.write("    return best;\n")
        f.write("}\n\n")
        ...
        f.write(f"inline std::array<float, INPUT_SIZE> extract_features(const std::string& seq) {{\n")
        ...
        f.write("        \n")
        f.write("        // Dipeptide entropy (simplified)\n")
        f.write("        features[idx++] = 0.5f;  // Placeholder - compute if needed\n")
        f.write("        \n")
        f.write("        // Hydrophobic fraction (simplified)\n")
        f.write("        features[idx++] = 0.3f;  // Placeholder\n")
        f.write("        \n")
        ...
        f.write("        size_t last_codon = ((strand.length() - bf) / 3 - 1) * 3 + bf;\n")
        ...
        f.write("            has_term_stop = (c1=='T' && c2=='A' && (c3=='A' || c3=='G')) ||\n")
        f.write("                           (c1=='T' && c2=='G' && c3=='A');\n")
        f.write("        }\n")
        ...
        f.write("                                  count_internal_stops(strand, 2)});\n")
        ...
        f.write("                                    compute_hexamer_score(seq, 2)});\n")
        ...
        # Forward pass
        f.write("// Predict strand: returns probability that FORWARD strand is correct\n")
        f.write("inline float predict_strand(const std::array<float, INPUT_SIZE>& features) {\n")
...
def main():
    parser = argparse.ArgumentParser(description='Train strand classifier on aMGSIM data')
    ...
    parser.add_argument('--max-train', type=int, default=100000, help='Max samples per training file')
    parser.add_argument('--max-test', type=int, default=10000, help='Max samples per test file')
    ...
    print(f"Forward: {fwd_count}, Reverse: {len(train_data) - fwd_count}", file=sys.stderr)
